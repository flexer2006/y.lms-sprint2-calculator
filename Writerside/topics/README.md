...
docker run -d -p 8080:8080 --name orchestrator --network \
calculator-network calculator-orchestrator  
```  
...
.\build.ps1 stop  
```  
**Запуск с большим количеством агентов:**  
```bash
...
```bash  
curl -L 'http://localhost:8080/api/v1/calculate' \
-H 'Content-Type: application/json' --data '{"expression":"2+2*2"}'  
```  
...
- Убедитесь, что порты не заблокированы firewall 
